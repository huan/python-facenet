...
def _chinese_whispers(encoding_list, threshold=0.75, iterations=20):
    ...
    Inputs:
        ...
        iterations: since chinese whispers is an iterative algorithm, number
        of times to iterate

    Outputs:
        sorted_clusters: a list of clusters, a cluster being a list of
            magepaths, sorted by largest cluster to smallest
    """
...
def cluster_facial_encodings(facial_encodings):
    """ Cluster facial encodings
        ...
        Intended to be an optional switch for different clustering algorithms,
        as of right now only chinese whispers is available.

        Input:
            facial_encodings: (image_path, facial_encoding) dictionary of
                facial encodings

        Output:
            sorted_clusters: a list of clusters, a cluster being a list of
                imagepaths, sorted by largest cluster to smallest

    """

    if len(facial_encodings) <= 1:
        print("Number of facial encodings must be greater than one,"
              " can't cluster")
        return []
...
def compute_facial_encodings(
        sess, images_placeholder, embeddings,
    ...
    """ Compute Facial Encodings
        ...
        Given a set of images, compute the facial encodings of each face
        detected in the images and
        return them. If no faces, or more than one face found,
        return nothing for that image.
...
def main(args):
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            #   facenet.get_image_paths_and_labels(train_set)
            ...
            for img_cls in train_set:
                print('img_cls', img_cls)
                counter += 1
                if counter < 56700:
                    continue
                if counter % 100 == 0:
                    print(counter)
                image_paths = img_cls.image_paths
                ...
                facial_encodings = compute_facial_encodings(
                    sess, images_placeholder, embeddings,
                    phase_train_placeholder, image_size,
                    embedding_size, nrof_images,
                    nrof_batches, emb_array,
                ...
                dest_dir = join(args.output, img_cls.name)
...
def parse_args():
    ...
    parser.add_argument('--model_dir', type=str,
                        help='model dir', required=True)
    parser.add_argument('--batch_size', type=int,
                        help='model dir', required=30)
    parser.add_argument('--input', type=str,
                        help='Input dir of images', required=True)
    parser.add_argument('--output', type=str,
                        help='Output dir of clusters', required=True)
